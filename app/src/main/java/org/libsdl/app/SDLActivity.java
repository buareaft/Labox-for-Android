...
***/
    ...
    /** 将 Android 输入法提交的文本直接交给 SDL，不在应用内保存或显示。 */
    ...
    /** 在 SDL 包内部创建 Surface，避免子类直接引用包内可见的 SDLSurface 类型。 */
    protected void createSurfaceIn(ViewGroup layout) {
        mSurface = new ExSDLSurface(this);
        mLayout = layout;
        layout.addView(mSurface, new ViewGroup.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT));
    }
    /** 让 SurfaceFlinger 将客体帧缓冲缩放到 SurfaceView，避免原始分辨率只占屏幕一角。 */
    protected void resizeSurfaceBuffer(int width, int height) {
